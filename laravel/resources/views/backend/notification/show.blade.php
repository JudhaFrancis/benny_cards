<div id="notifications">
    @php
    use App\Models\Notification;

    // 🧠 Fetch failed notifications (latest 5)
    $notifications = Notification::where('status', 'failed')
    ->orderByDesc('created_at')
    ->limit(5)
    ->get();

    // 🔢 Count unread notifications
    $unreadCount = Notification::where('status', 'failed')->count();
    @endphp

    <!-- Bell Icon with Counter -->
    <a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button"
        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <i class="fas fa-bell fa-fw"></i>

        <!-- Counter - Alerts -->
        <span class="badge badge-danger badge-counter">
            @if($unreadCount > 5)
            <span data-count="5" class="count">5+</span>
            @else
            <span class="count" data-count="{{ $unreadCount }}">{{ $unreadCount }}</span>
            @endif
        </span>
    </a>

    <!-- Dropdown - Alerts -->
    <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
        aria-labelledby="alertsDropdown">

        <h6 class="dropdown-header">
            Notifications Center
        </h6>

        @forelse($notifications as $notification)
        <a class="dropdown-item d-flex align-items-center"
            target="_blank"
            href="{{ route('admin.notification', $notification->id) }}">
            <div class="mr-3">
                <div class="icon-circle 
                        @if($notification->message_type == 'order') bg-success 
                        @elseif($notification->message_type == 'alert') bg-warning 
                        @else bg-primary 
                        @endif">
                    <i class="fas fa-bell text-white"></i>
                </div>
            </div>
            <div>
                <div class="small text-gray-500">
                    {{ $notification->created_at->format('F d, Y h:i A') }}
                </div>
                <span class="font-weight-bold">{{ $notification->message }}</span>
            </div>
        </a>
        @empty
        <div class="dropdown-item text-center small text-gray-500">
            No new notifications
        </div>
        @endforelse

        <a class="dropdown-item text-center small text-gray-500" href="{{ route('all.notification') }}">
            Show All Notifications
        </a>
    </div>
</div>